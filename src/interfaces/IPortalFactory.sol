...
interface IPortalFactory {
    event PortalCreated(address indexed portal, address indexed operator, address paymentToken, bytes peerId);

    event PortalUpgraded(address indexed portal, address indexed newImplementation);

    event StakeMoved(address indexed fromPortal, address indexed toPortal, address indexed provider, uint256 amount);
    ...
    function moveStake(address fromPortal, address toPortal, uint256 amount) external;
