...
export function MintUSDC() {
  ...
  return <MintUSDCModal onClose={() => setShowModal(false)} />;
}

function MintUSDCModal({ onClose }: { onClose: () => void }) {
  const { address } = useAccount();
  const chainId = useChainId();
  const [recipient, setRecipient] = useState("0xd409943eD69aDe02d0B25D0cbc47dc43b7391c34");
  const [amount, setAmount] = useState("1000000"); // 1 million USDC (6 decimals)
      ...
      <div className="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-sqd-divider p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-sqd-text-primary mb-1">Mint USDC Tokens</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-sqd-text-secondary mt-1">
              Enter amount without decimals (e.g., 1000000 for 1M USDC, 6 decimals)
            </p>
          </div>
          ...
          {balance && (
            <div className="p-3 bg-sqd-primary rounded-lg">
              ...
              <div className="text-lg font-semibold text-sqd-accent">
                {formatUnits(balance, 6).replace(/\.?0+$/, "")} USDC
              </div>
            </div>
          )}
