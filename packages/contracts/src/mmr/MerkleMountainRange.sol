...
library MerkleMountainRange {
  ...
  function getSubtreeLeaves(MmrLeaf[] memory leaves, LeafIterator memory leafIter, uint256 current_subtree)
    internal
    pure
  ...
  function mmrLeafToNode(MmrLeaf[] memory leaves, LeafIterator memory leafIter)
    internal
    pure
    returns (Node[] memory, uint256[] memory)
  {
  ...
  function leavesForSubtree(MmrLeaf[] memory leaves, uint256 leafIndex)
    internal
    pure
    returns (LeafIterator memory, LeafIterator memory)
  {
    uint256 p;
