...
export class EpochProcessorService {
  ...
  async processEpoch(
    ...
    const fromBlockTimestamp = await this.web3Service.getBlockTimestamp(
      ctx,
      fromBlock,
    );
    const toBlockTimestamp = await this.web3Service.getBlockTimestamp(
      ctx,
      toBlock,
    );
    ...
    try {
      ...
      // success - collect metrics (rewards report handled by distributeEpochRewards)
      ...
      // Note: rewards report is generated by distributeEpochRewards() to avoid duplicates
