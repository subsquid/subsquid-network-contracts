...
export class BlockSchedulerService {
  ...
  constructor(
    ...
    this.blockInterval = this.configService.get('rewards.distributionBlockInterval') || 600;
    this.enableAutoDistribution = this.configService.get('rewards.enableAutoDistribution') === 'true';
    this.confirmationBlocks = this.configService.get('blockchain.epochConfirmationBlocks') || 150;
    
    this.logger.log(`🔄 Block Scheduler initialized:`);
    this.logger.log(`   - Auto distribution: ${this.enableAutoDistribution ? '✅' : '❌'}`);
  ...
  private async processBlockInterval(): Promise<void> {
    ...
    try {
      ...
      if (commitRange.fromBlock > 0 && commitRange.toBlock > 0) {
        this.logger.log(`🚀 Block interval reached! Processing range ${commitRange.fromBlock}-${commitRange.toBlock}`);
        

        await this.processCommitPhase(commitRange.fromBlock, commitRange.toBlock);
        
        await this.processApprovalPhase(commitRange.fromBlock, commitRange.toBlock);
        
        await this.processDistributionPhase(commitRange.fromBlock, commitRange.toBlock);
        
        this.logger.log(`✅ Block-triggered workflow completed for ${commitRange.fromBlock}-${commitRange.toBlock}`);
      } else {
        ...
        this.logger.debug(`📊 Block status: current=${currentBlock}, lastRewarded=${lastRewardedBlock}, gap=${blocksSinceLastReward}/${this.blockInterval}`);
        
        if (blocksSinceLastReward < this.blockInterval) {
          this.logger.debug(`⏳ Waiting for more blocks (${this.blockInterval - blocksSinceLastReward} remaining)`);
        }
      }
  ...
  private async processCommitPhase(fromBlock: number, toBlock: number): Promise<void> {
    try {
      this.currentPhase = 'commit';
      this.logger.log(`📝 Phase 1: Committing Merkle root for ${fromBlock}-${toBlock}`);
      
      const isCommitted = await this.contractService.isCommitted(fromBlock, toBlock);
      if (isCommitted) {
      ...
      if (!canCommit) {
        this.logger.warn(`⚠️  Cannot commit right now (not our turn in round-robin)`);
        return;
      }
      ...
      if (workers.length === 0) {
        this.logger.warn(`⚠️  No workers found for range ${fromBlock}-${toBlock}, skipping commit`);
        return;
      }
      ...
      this.logger.log(`📤 Committing Merkle root ${merkleTree.root} with ${merkleTree.totalBatches} batches...`);
  ...
  private async processApprovalPhase(fromBlock: number, toBlock: number): Promise<void> {
    try {
      this.currentPhase = 'approve';
      this.logger.log(`✅ Phase 2: Checking approvals for ${fromBlock}-${toBlock}`);
      ...
      this.logger.log(`📊 Approval status: ${currentApprovals}/${requiredApprovals} required`);

      if (currentApprovals < requiredApprovals) {
        // if we haven't approved yet, approve now
        const distributorAddress = this.configService.get('blockchain.distributor.address');
        
        this.logger.log(`📝 Approving commitment for ${fromBlock}-${toBlock}...`);
  ...
  private async processDistributionPhase(fromBlock: number, toBlock: number): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`💰 Phase 3: Distributing rewards for ${fromBlock}-${toBlock}`);
      ...
      if (distributionStatus.status === 'completed') {
        ...
        const baseApr = 0.20; // 20% base APR
  ...
  private async getCommitRange(): Promise<{ fromBlock: number; toBlock: number }> {
    try {
      const currentBlock = await this.web3Service.getL1BlockNumber();
      const lastRewardedBlock = await this.contractService.getLastRewardedBlock();
  ...
  async forceDistribution(fromBlock: number, toBlock: number): Promise<boolean> {
    try {
      this.logger.log(`🔧 Force distribution initiated for ${fromBlock}-${toBlock}`);
      this.isProcessing = true;
