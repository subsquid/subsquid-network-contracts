...
export class BlockSchedulerService implements OnModuleInit {
  ...
  constructor(
    ...
    const distributionInterval = this.configService.get(
      'rewards.distributionBlockInterval',
      520,
    );
    ...
    ctx.logger.info(
      `   - Distribution interval: ${distributionInterval} blocks (~${Math.round((distributionInterval * 12) / 60)} minutes)`,
    );
    ctx.logger.info(
      `   - Round robin window: ${roundRobinWindow} blocks (~${Math.round((roundRobinWindow * 12) / 60)} minutes)`,
    );
    ctx.logger.info(`   - Windows per cycle: ${windowsPerCycle}`);
    ctx.logger.info(
      `   - Commit safety buffer: ${safetyBuffer} blocks (~${safetyBuffer * 12} seconds)`,
    );
  }
  ...
  async checkDistributionInterval() {
    ...
    try {
      ...
      if (committerCheck.isCommitter) {
        ctx.logger.info(
          `🎯 I'm committer - checking commitment and approval status`,
        );
        ...
        if (!commitmentStatus.exists) {
          ...
          if (current >= required) {
            ctx.logger.info(
              `✅ Enough approvals (${current}/${required}) + canCommit=true - STARTING DISTRIBUTION`,
            );
            await this.epochProcessor.processDistribution();
          } else {
            ctx.logger.info(
              `⏳ Waiting for approvals: ${current}/${required} (need ${required - current} more)`,
            );
          }
        }
      } else {
        ctx.logger.debug(
          `⏳ Not current committer - skipping distribution check`,
        );
      }
  ...
  async checkRecoveryInterval() {
    ...
    try {
      ...
      if (shouldSkipRecovery) {
        ctx.logger.debug(
          '🚫 Skipping recovery - conditions not met or another bot is active',
        );
        return;
      }

      ctx.logger.info(
        '🔧 Recovery conditions met - attempting recovery distribution',
      );
