...
export class DistributionService {
  ...
  private readonly gasSimulationConfig = {
    ...
    maxOptimizationAttempts: parseInt(process.env.MAX_GAS_OPTIMIZATION_ATTEMPTS || '10'),
  ...
  constructor(
    ...
    this.logger.log(`  - Enabled: ${this.gasSimulationConfig.enablePreflightSimulation}`);
    this.logger.log(`  - Max optimization attempts: ${this.gasSimulationConfig.maxOptimizationAttempts}`);
    this.logger.log(`  - Min batch size: ${this.gasSimulationConfig.minBatchSize}`);
    this.logger.log(`  - Gas reduction factor: ${this.gasSimulationConfig.gasReductionFactor}`);
  }
    ...
    try {
      ...
      if (this.gasSimulationConfig.enablePreflightSimulation) {
        ...
        if (optimizedBatchSize !== batchSize) {
          this.logger.warn(`⚠️ Batch size adjusted from ${batchSize} to ${optimizedBatchSize} based on gas simulation`);
        } else {
          this.logger.log(`✅ Original batch size ${batchSize} is optimal`);
        }
      } else {
        this.logger.log(`⚠️ Gas simulation disabled, using original batch size ${batchSize}`);
      }
      ...
      const merkleTree = await this.merkleTreeService.generateMerkleTree(workerRewards, optimizedBatchSize);
      
      status.totalBatches = merkleTree.totalBatches;
      status.merkleRoot = merkleTree.root;

      this.logger.log(`✅ Generated optimized Merkle tree: root=${merkleTree.root}, batches=${merkleTree.totalBatches} (gas simulations: ${totalGasSimulations})`);
  ...
  private async simulateDistribution(
    ...
    try {
      this.logger.debug(`🧪 Simulating distribution for ${recipients.length} workers...`);
      ...
      this.logger.debug(`✅ Gas simulation successful: ${gasEstimate} gas for ${recipients.length} workers`);
      ...
      this.logger.warn(`❌ Gas simulation failed for ${recipients.length} workers: ${errorMessage}`);
  ...
  private async optimizeBatchSize(
    ...
    this.logger.log(`🔧 Optimizing batch size starting with ${initialBatchSize} workers per batch...`);
    ...
    while (gasSimulations < maxAttempts) {
      const testBatch = workers.slice(0, Math.min(testBatchSize, workers.length));
      
             if (testBatch.length === 0 || testBatchSize < this.gasSimulationConfig.minBatchSize) {
         this.logger.warn(`⚠️ Test batch size reduced below minimum (${this.gasSimulationConfig.minBatchSize}), using minimum size`);
         return { optimizedBatchSize: this.gasSimulationConfig.minBatchSize, gasSimulations };
       }

      const testMerkleTree = await this.merkleTreeService.generateMerkleTree(testBatch, testBatchSize);
      ...
      if (simulationResult.success) {
        this.logger.log(`✅ Optimal batch size found: ${testBatchSize} workers (${gasSimulations} simulations)`);
        return { optimizedBatchSize: testBatchSize, gasSimulations };
      } else {
        this.logger.warn(`❌ Batch size ${testBatchSize} failed simulation: ${simulationResult.error}`);
        
        if (simulationResult.suggestedBatchSize && simulationResult.suggestedBatchSize < testBatchSize) {
          testBatchSize = simulationResult.suggestedBatchSize;
                 } else {
           // fallback: reduce by configured factor
           testBatchSize = Math.max(this.gasSimulationConfig.minBatchSize, Math.floor(testBatchSize * this.gasSimulationConfig.gasReductionFactor));
         }

        this.logger.log(`🔄 Retrying with reduced batch size: ${testBatchSize}`);
      }
    }

         this.logger.warn(`⚠️ Could not find optimal batch size after ${maxAttempts} attempts, defaulting to minimum size ${this.gasSimulationConfig.minBatchSize}`);
     return { optimizedBatchSize: this.gasSimulationConfig.minBatchSize, gasSimulations };
  }
  ...
  private async distributeBatches(
    ...
    this.logger.log(`Distributing ${merkleTree.totalBatches} batches with gas simulation...`);
    ...
    for (let i = 0; i < merkleTree.leaves.length; i++) {
      ...
      try {
        this.logger.log(`📦 Processing batch ${i + 1}/${merkleTree.totalBatches} with ${leaf.recipients.length} workers`);
        ...
        if (this.gasSimulationConfig.enablePreflightSimulation) {
          ...
          if (!gasSimulation.success) {
            this.logger.error(`❌ Pre-flight simulation failed for batch ${i + 1}: ${gasSimulation.error}`);
            

            if (leaf.recipients.length > 1) {
              this.logger.warn(`🔄 Attempting to split failing batch ${i + 1} into smaller chunks...`);
              await this.distributeBatchInChunks(fromBlock, toBlock, leaf, proof, i + 1);
              continue;
            } else {
              // up to single worker
              throw new Error(`Single worker batch failed simulation: ${gasSimulation.error}`);
            }
          }

          this.logger.log(`🧪 Pre-flight simulation passed for batch ${i + 1}, estimated gas: ${gasSimulation.estimatedGas}`);
        }
        ...
        this.logger.log(`✅ Batch ${i + 1} distributed successfully: tx=${receipt.transactionHash}, gas used: ${receipt.gasUsed}`);
  ...
  private async distributeBatchInChunks(
    fromBlock: number,
    toBlock: number,
    originalLeaf: { recipients: bigint[]; workerRewards: bigint[]; stakerRewards: bigint[] },
    originalProof: string[],
    batchNumber: number
  ): Promise<void> {
    const chunkSize = Math.max(1, Math.floor(originalLeaf.recipients.length / 2));
    this.logger.log(`📦 Splitting batch ${batchNumber} into chunks of ${chunkSize} workers each`);

    for (let chunkStart = 0; chunkStart < originalLeaf.recipients.length; chunkStart += chunkSize) {
      const chunkEnd = Math.min(chunkStart + chunkSize, originalLeaf.recipients.length);
      
      const chunkRecipients = originalLeaf.recipients.slice(chunkStart, chunkEnd);
      const chunkWorkerRewards = originalLeaf.workerRewards.slice(chunkStart, chunkEnd);
      const chunkStakerRewards = originalLeaf.stakerRewards.slice(chunkStart, chunkEnd);

      const chunkIndex = Math.floor(chunkStart / chunkSize) + 1;
      const totalChunks = Math.ceil(originalLeaf.recipients.length / chunkSize);

      this.logger.log(`📦 Processing chunk ${chunkIndex}/${totalChunks} of batch ${batchNumber} with ${chunkRecipients.length} workers`);
      ...
      try {
        ...
        if (!chunkSimulation.success) {
          this.logger.error(`❌ Chunk ${chunkIndex} simulation failed: ${chunkSimulation.error}`);
          
          if (chunkRecipients.length === 1) {
            this.logger.error(`❌ Single worker chunk failed - skipping worker ${chunkRecipients[0]}`);
            continue;
          } else {
        ...
        this.logger.log(`🧪 Chunk ${chunkIndex} simulation passed, estimated gas: ${chunkSimulation.estimatedGas}`);
        ...
        this.logger.log(`✅ Chunk ${chunkIndex}/${totalChunks} of batch ${batchNumber} distributed: tx=${receipt.transactionHash}, gas used: ${receipt.gasUsed}`);

      } catch (error) {
        this.logger.error(`❌ Failed to distribute chunk ${chunkIndex} of batch ${batchNumber}: ${error.message}`);
        throw error;
      }
    }
  }
  ...
  private logGasOptimizationSummary(gasOptimizations: {
    ...
    this.logger.log(`   - Original batch size: ${gasOptimizations.originalBatchSize} workers`);
    this.logger.log(`   - Final batch size: ${gasOptimizations.finalBatchSize} workers`);
    this.logger.log(`   - Batches adjusted: ${gasOptimizations.batchesAdjusted}`);
    this.logger.log(`   - Total gas simulations: ${gasOptimizations.totalGasSimulations}`);
    
    if (gasOptimizations.originalBatchSize !== gasOptimizations.finalBatchSize) {
      const reduction = ((gasOptimizations.originalBatchSize - gasOptimizations.finalBatchSize) / gasOptimizations.originalBatchSize * 100).toFixed(1);
