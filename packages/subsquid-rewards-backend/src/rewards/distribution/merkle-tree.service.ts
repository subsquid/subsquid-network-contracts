...
export class MerkleTreeService {
  ...
  async generateMerkleTree(
    ...
    this.logger.log(`Generating Merkle tree for ${workers.length} workers with batch size ${batchSize}`);
    ...
    this.logger.log(`Created ${batches.length} batches from ${workers.length} workers`);
    ...
    const leafHashes = batches.map((batch, index) => {
      ...
      this.logger.debug(`Batch ${index}: ${batch.recipients.length} workers, hash: ${hash}`);
      return hash;
    });
    ...
    this.logger.log(`✅ Merkle tree generated: root=${root}, ${batches.length} leaves`);
  ...
  private buildMerkleTree(leaves: string[]): { root: string; proofs: string[][] } {
    if (leaves.length === 0) {
    ...
    while (currentLevel.length > 1) {
      ...
      for (let i = 0; i < currentLevel.length; i += 2) {
        ...
        const right = (i + 1 < currentLevel.length ? currentLevel[i + 1] : left) as `0x${string}`;
        
        // sort hashes to ensure deterministic tree
        const [sortedLeft, sortedRight] = left <= right ? [left, right] : [right, left];
        const combined = keccak256(encodePacked(['bytes32', 'bytes32'], [sortedLeft, sortedRight]));
  ...
  getTotalRewards(leaves: MerkleLeaf[]): { totalWorkerRewards: bigint; totalStakerRewards: bigint } {
    let totalWorkerRewards = 0n;
    let totalStakerRewards = 0n;

    for (const leaf of leaves) {
      totalWorkerRewards += leaf.workerRewards.reduce((sum, reward) => sum + reward, 0n);
      totalStakerRewards += leaf.stakerRewards.reduce((sum, reward) => sum + reward, 0n);
    }
