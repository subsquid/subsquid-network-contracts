...
describe('MerkleTreeService', () => {
  ...
  describe('generateBatchHash', () => {
    it('should generate correct batch hash for contract compatibility', () => {
      ...
      const workerRewards = ['1000000000000000000', '2000000000000000000', '3000000000000000000']; // 1, 2, 3 SQD
      const stakerRewards = ['100000000000000000', '200000000000000000', '300000000000000000']; // 0.1, 0.2, 0.3 SQD

      const hash = service.generateBatchHash(recipients, workerRewards, stakerRewards);
    ...
    it('should generate different hashes for different batches', () => {
      ...
      const hash1 = service.generateBatchHash(batch1.recipients, batch1.workerRewards, batch1.stakerRewards);
      const hash2 = service.generateBatchHash(batch2.recipients, batch2.workerRewards, batch2.stakerRewards);
  ...
  describe('createBatches', () => {
    it('should create batches with correct structure', () => {
      const workers = [
        ...
        { workerId: 4, workerReward: '4000000000000000000', stakerReward: '400000000000000000' },
        { workerId: 5, workerReward: '5000000000000000000', stakerReward: '500000000000000000' },
      ];
      ...
      expect(batches[0].workerRewards).toEqual(['1000000000000000000', '2000000000000000000']);
      expect(batches[0].stakerRewards).toEqual(['100000000000000000', '200000000000000000']);
  ...
  describe('buildMerkleTree', () => {
    ...
    it('should match reference implementation from generate_merkle.mjs', () => {
      ...
      const testBatches = [
        {
          batchId: 0,
          recipients: [1, 2, 3],
          workerRewards: ['1000000000000000000', '2000000000000000000', '1500000000000000000'],
          stakerRewards: ['100000000000000000', '200000000000000000', '150000000000000000'],
          leafHash: ''
        },
        {
          batchId: 1,
        ...
        },
        {
          batchId: 2,
          recipients: [6, 7, 8],
          workerRewards: ['1800000000000000000', '2200000000000000000', '1600000000000000000'],
          stakerRewards: ['180000000000000000', '220000000000000000', '160000000000000000'],
          leafHash: ''
        },
      ];
      ...
      testBatches.forEach(batch => {
        batch.leafHash = service.generateBatchHash(batch.recipients, batch.workerRewards, batch.stakerRewards);
      });
  ...
  describe('verifyProof', () => {
    it('should reject invalid proofs', () => {
      ...
      const wrongLeaf = '0x1234567890123456789012345678901234567890123456789012345678901234';
      const isValid = service.verifyProof(
        wrongLeaf,
        merkleData.proofs[0],
        merkleData.root
      ...
      const wrongProof = ['0x1234567890123456789012345678901234567890123456789012345678901234'];
