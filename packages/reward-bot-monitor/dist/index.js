...
const tokenAddress = chainId === arbitrum.id
    ? "0x1337420dED5ADb9980CFc35f8f2B054ea86f8aB1"
    : "0x24f9C46d86c064a6FA2a568F918fe62fC6917B3c";
const ethMulticalls = ethBalanceWallets.map((address) => ({
    address: client.chain.contracts.multicall3.address,
    abi: multicallAbi,
    functionName: "getEthBalance",
    args: [address],
}));
const tokenMulticalls = sqdBalanceWallets.map((address) => ({
    ...
    for (const balance of balances) {
        if (balance.status === "success") {
            if (i < ethMulticalls.length) {
                balanceGauge.set({ wallet: ethBalanceWallets[i], token: "ETH" }, Number(formatEther(balance.result)));
            }
            else {
                balanceGauge.set({ wallet: sqdBalanceWallets[i - ethMulticalls.length], token: "SQD" }, Number(formatEther(balance.result)));
            }
        }
        i++;
    }
    setTimeout(updateMetrics, 1000 * 60 * Number(process.env.INTERVAL_MINUTES ?? 120));
};
void updateMetrics();
